...
def finetune(
    *,
    ...
    wandb.init(project=wandb_project, config=locals(), name=f"finetune timesfm context {context_len} horizon {horizon_len}")

    data_df = pd.read_csv(open(data_path, "r"))
    # For fintuning, we finetune with all previous data to forecast the next 4 weeks.
    # Start with the 17th week so the model has enough data to learn from, smallest context is 16 weeks.
    # We test all weeks from the 17th to the last.
    # For zero-shot, we use just 8 weeks of data to forecast the next 4 weeks.
    # We test all weeks from the 9th to the last.
    # Each 4 weeks forecast is tested using RMSE and MAPE for 1, 2, 3 and 4 weeks.
    ...
    train_batches = dtl.tf_dataset(mode="train", shift=1).batch(batch_size, drop_remainder=True)
    val_batches = dtl.tf_dataset(mode="val", shift=horizon_len)
    test_batches = dtl.tf_dataset(mode="test", shift=horizon_len)

    # for vbatch in tqdm(val_batches.as_numpy_iterator()):
    #     print(vbatch[0].shape)
    #     break
    ...
    hparams = TimesFmHparams(context_len=context_len,
    ...
    ckpt = TimesFmCheckpoint(huggingface_repo_id="google/timesfm-1.0-200m")
    ...
    tfm = TimesFm(hparams=hparams, checkpoint=ckpt)
    ...
    for batch in tqdm(test_batches.as_numpy_iterator()):
        ...
        forecasts = forecasts[:, 0 : actuals.shape[1], 1]
        # Calculate RMSE and MAPE for 1, 2, 3, 4 weeks
        temp_RMSEs = []
        temp_MAPEs = []
        for i in range(4):
            first_i_week_forecasts = forecasts[:, :(i+1)*7]
            first_i_week_actuals = actuals[:, :(i+1)*7]
            temp_RMSEs.append(np.sqrt(np.mean((first_i_week_forecasts - first_i_week_actuals) ** 2)))
            temp_MAPEs.append(np.mean(np.abs(first_i_week_forecasts - first_i_week_actuals) / (first_i_week_actuals + EPS)))
        RMSEs.append(temp_RMSEs)
        MAPEs.append(temp_MAPEs)
        
    RMSEs = np.array(RMSEs)
    MAPEs = np.array(MAPEs)
    # Calculate the mean RMSE and MAPE for 1, 2, 3, 4 weeks
    mean_RMSEs = RMSEs.mean(axis=0)
    mean_MAPEs = MAPEs.mean(axis=0)
    print(f"Mean RMSE for 1, 2, 3, 4 weeks: {mean_RMSEs}")
    print(f"Mean MAPE for 1, 2, 3, 4 weeks: {mean_MAPEs}")
    ...
    task_p.model.ici_mesh_shape = [2, 2, 2]
    ...
    def eval_step(states, prng_key, inputs):
        ...
        # print("input shape", inputs.input_ts.shape)
    ...
    for epoch in range(num_epochs):
        ...
        for ev_batch in tqdm(val_its):
            ...
            if ebatch.input_ts.shape[0] < num_devices:
                continue
            ebatch = reshape_batch_for_pmap(ebatch, num_devices)
            # print("input shape", ebatch.input_ts.shape)
