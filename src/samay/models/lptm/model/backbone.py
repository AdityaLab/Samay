...
class TimeseriesOutputs:
    forecast: Optional[npt.NDArray] = None
    anomaly_scores: Optional[npt.NDArray] = None
    logits: Optional[npt.NDArray] = None
    labels: Optional[int] = None
    input_mask: Optional[npt.NDArray] = None
    pretrain_mask: Optional[npt.NDArray] = None
    reconstruction: Optional[npt.NDArray] = None
...
class ClassificationHead(nn.Module):
    def __init__(
        ...
        if reduction == "mean":
            ...
            raise ValueError(
                f"Reduction method {reduction} not implemented. Only 'mean' and 'concat' are supported."
            )
...
class LPTMPipeline(LPTM, PyTorchModelHubMixin):
    def __init__(self, config: Namespace | dict, **kwargs: dict):
