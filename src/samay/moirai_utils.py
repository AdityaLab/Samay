...
from .models.uni2ts.distribution.mixture import MixtureOutput
                        ...
                        if k == "start":
                            mod_row[i]["start"] = (
                                ...
                                if isinstance(v, torch.Tensor)
                                    ...
                                    v.to_timestamp().timestamp(), dtype=torch.float32
                                )
                            )
                        elif k == "forecast_start":
                            mod_row[i]["forecast_start"] = (
                                v
                                if isinstance(v, torch.Tensor)
                                else torch.tensor(
                                    v.to_timestamp().timestamp(), dtype=torch.float32
