...
def train_model(
    ...
    **kwargs,
    ...
    # Update default configuration with provided arguments, overriding defaults where necessary
    ...
    if logger is None:
        ...
        if not logger.handlers:
            # Create console handler if no handlers are present
            ch = logging.StreamHandler()
            ch.setLevel(logging.INFO)
            logger.addHandler(ch)

    # Check for TF32 support and adjust
    if config['tf32'] and not (torch.cuda.is_available() and torch.cuda.get_device_capability()[0] >= 8):
        logger.warning("TF32 is not supported on this system, disabling tf32.")
        config['tf32'] = False
    ...
    if config['dataloader_num_workers'] > len(training_data_paths):
        logger.warning(
            "dataloader_num_workers is bigger than number of datasets, "
            "setting it to the number of datasets."
        )
        config['dataloader_num_workers'] = len(training_data_paths)

    assert config['model_type'] in ["seq2seq", "causal"]

    output_dir = get_next_path("run", base_dir=output_dir, file_type="")

    logger.info(f"Logging dir: {output_dir}")
    logger.info(
    ...
    shuffled_train_dataset = ChronosDataset(
        ...
        model_type=config['model_type'],
        imputation_method=LastValueImputation() if config['model_type'] == "causal" else None,
        mode="training",
    ).shuffle(shuffle_buffer_length=config['shuffle_buffer_length'])
    ...
    if is_main_process():
        ...
        logger.info(f"Model saved to {final_checkpoint_dir}")

        # Save training info
        save_training_info(output_dir, config)
...
#         job_info["world_size"] = dist.get_world_size()
#     else:
#         job_info["world_size"] = 1  # Default to 1 if not using distributed training
...
def save_training_info(ckpt_path: Path, training_config: Dict):
    ...
    # Convert PosixPath objects to strings and exclude Logger objects and "handler"
    training_config = {k: str(v) if isinstance(v, Path) else v for k, v in training_config.items() if not isinstance(v, logging.Logger) and k != "handler"}
    job_info = get_training_job_info()
    job_info = {k: str(v) if isinstance(v, Path) else v for k, v in job_info.items()}

    # Log the seed
    if "seed" in training_config:
        logging.info(f"Seed: {training_config['seed']}")
        return  # Break out after logging the seed

    with open(ckpt_path / "training_info.json", "w") as fp:
        json.dump(
            {"training_config": training_config, "job_info": job_info},
            fp,
            indent=4,
        )
...
# Modify the main function to call the new train_model function
@app.command()
...
def main(
    ...
    probability = ast.literal_eval(probability) if probability else None
